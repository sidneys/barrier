...
App::version()
{
    std::cout << argsBase().m_pname << " " << kVersion << ", protocol version " << kProtocolMajorVersion << "." << kProtocolMinorVersion << std::endl;
...
App::initApp(int argc, const char** argv)
{
    ...
    if (!CLOG->setFilter(argsBase().m_logFilter)) {
        LOG((CLOG_PRINT "%s: unrecognized log level `%s'" BYE,
            argsBase().m_pname.c_str(), argsBase().m_logFilter, argsBase().m_pname.c_str()));
        m_bye(kExitArgs);
    }
