...
ArgParser::parseServerArgs(ServerArgs& args, int argc, const char* const* argv)
{
    ...
    for (int i = 1; i < argc; ++i) {
        if (parsePlatformArg(args, argc, argv, i)) {
            ...
            LOG((CLOG_PRINT "%s: unrecognized option `%s'" BYE, args.m_pname.c_str(), argv[i], args.m_pname.c_str()));
            return false;
        }
    }
...
ArgParser::parseClientArgs(ClientArgs& args, int argc, const char* const* argv)
{
    ...
    if (i == argc) {
        LOG((CLOG_PRINT "%s: a server address or name is required" BYE,
            args.m_pname.c_str(), args.m_pname.c_str()));
        return false;
    }
        ...
        (name2 != NULL && strcmp(argv[argi], name2) == 0)) {
            ...
            if (argi + minRequiredParameters >= argc) {
                LOG((CLOG_PRINT "%s: missing arguments for `%s'" BYE,
                    argsBase().m_pname.c_str(), argv[argi], argsBase().m_pname.c_str()));
                argsBase().m_shouldExit = true;
                return false;
            }
            return true;
...
ArgParser::updateCommonArgs(const char* const* argv)
{
    ...
    argsBase().m_pname = PathUtilities::basename(argv[0]);
}
